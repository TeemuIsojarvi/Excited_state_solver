...
const double L = 1.6;     // Spatial domain size [-L/2, L/2] in both dimensions
const double TIME_T = 3.141/46.0; // Total time interval for one iteration
double DT = 0.004;   // Time step size
const int NT = static_cast<int>(TIME_T / DT); // Number of time steps
const int N_ITERATIONS = 1000; // Total number of outer iterations for convergence
constexpr std::array<int, 4> Refine_points = {800, 940, 980, 992}; // Iteration numbers where discretization is refined

// Initial grid points (Coarse grid)
const int NX_COARSE = 100;
const int NY_COARSE = 100;

const double E_trial = 46.0; // Attempting to converge to an eigenstate of H with energy closest to this value

// Potential energy for a particle in a rectangle and finite potential step at boundaries
double potential(double x, double y) {
    // Return a potential energy with value V(x,y) = -E_trial when inside a rectangle of side lengths 1.3 and 1.0
    // and value V(x,y) = 2*10^5 elsewhere.
    if(abs(x) < 0.65 && abs(y) < 0.5) return -E_trial;
    else return 200000.0;
}
...
int main() {
    ...
    // 1. Initialize the wave function (sum of 2D Gaussians). This should not be anywhere near orthogonal with the target eigenstate.
    vector<Complex> psi(params.NTOTAL);
    double initial_width = L / 12.0;
    for (int jy = 0; jy < params.NY; ++jy) {
        double y = jy * params.DY - L / 2.0;
        for (int jx = 0; jx < params.NX; ++jx) {
            double x = jx * params.DX - L / 2.0;
            ...
            double gaussian = exp(-((x-0.1) * (x-0.1) + (y+0.15) * (y+0.15)) / (2.0 * initial_width * initial_width)) - 0.7*exp(-(x * x + (y-0.1) * (y-0.1)) / (2.0 * initial_width * initial_width));
