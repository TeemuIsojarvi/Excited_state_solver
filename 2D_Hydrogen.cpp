...
const double L = 16.0;     // Spatial domain size [-L/2, L/2] in both dimensions
const double TIME_T = 3.141; // Total time interval for one iteration
double DT = 0.05;   // Time step size
const int NT = static_cast<int>(TIME_T / DT); // Number of time steps
const int N_ITERATIONS = 15; // Total number of outer iterations for convergence
constexpr std::array<int, 1> Refine_points = {10}; // Iteration numbers where resolution is multiplied x1.5

// Initial grid points (Coarse grid)
const int NX_COARSE = 400;
const int NY_COARSE = 400;

const double E_trial = -1.0; // Attempting to converge to an eigenstate of H with energy closest to this value

double potential(double x, double y) {
    // 2D Hydrogen atom
    return -1.0/(sqrt(x*x + y*y)+0.0001) - E_trial;
}
...
int main() {
    ...
    for (int jy = 0; jy < params.NY; ++jy) {
        ...
        for (int jx = 0; jx < params.NX; ++jx) {
            ...
            double gaussian = exp(-((x-0.45) * (x-0.45) + (y+0.0) * (y+0.0)) / (2.0 * initial_width * initial_width)) - exp(-((x+0.45) * (x+0.45) + (y+0.0) * (y+0.0)) / (2.0 * initial_width * initial_width));
